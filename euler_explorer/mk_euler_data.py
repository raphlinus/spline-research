...
raw = []
raw2 = []
for line in open('/tmp/arms.dat'):
	s = line.split()
	if len(s) == 4:
		raw.append(float(s[2]))
		raw2.append(float(s[3]))
n = int(math.sqrt(len(raw)))

jsfile = open('euler_data.js', 'w');
jsfile.write('let rawDataN = ' + str(n) + ';\n')
jsfile.write('let rawData = new Float64Array([\n')
for i in range(len(raw)//2, len(raw)):
	jsfile.write(str(raw[i]) + ',\n')
for i in range(len(raw)//2, len(raw)):
...
imgfile = open('map.ppm', 'wb')
imgfile.write(bytes('P6\n' + str(n) + ' ' + str(n)  + '\n255\n', 'utf-8'))
for y in range(n):
	for x in range(n):
		z = raw[x * n + n - 1 - y]
